...
// Jacobi Method
double iterateJ(vector<vector<double>> &V, double V0) {
  auto Vtmp = V;
  double dVmax = 1e-50;
  int nx = V.size();
  int ny = V[0].size();
...
// Gauss-Seidel Method
double iterateGS(vector<vector<double>> &V, double V0) {
  double dVmax = 1e-50;
  int nx = V.size();
  int ny = V[0].size();
  for(int i = 1; i < nx - 1; i++) {
...
TGraph2D* LaplaceLine(int maxIter = 100, double eps = 0.001, int Npts = 100,
  ...
  int niter = 0;
  do {
    dV = iterateJ(V, V0);   // iterate using Jacobi method
    //dV=iterateGS(V, V0);   // iterate using Gauss-Seidel method
    ++niter;
    if (tc) {
      tc->cd();
